...
const ItemCheckOut = ({product}) => {
    return(
        <div className="w-full flex flex-row border-b-[1px] border-[#C4C4C4] mt-4 font-Public">
            <img className="desktop:w-24 ipad:w-20 h-full rounded-lg  mb-2" src={product.productImage} alt="img" />
            <div className="w-full flex flex-col justify-between pl-2 mb-2">
                ...
                <div className="flex flex-row gird grid-cols-2 justify-between mt-1">
                    ...
                    {/* Giá sản phẩm */}
                    <div className=" text-[#c73659] desktop:text-sm ipad:text-xs font-bold ">{Number((product.cost - (product.cost * product.discount / 100))).toLocaleString()}đ</div>
                </div>
            </div>
